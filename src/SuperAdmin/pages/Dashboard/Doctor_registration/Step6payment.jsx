...
export default function Step6Payment() {
  ...
  const basePrice = selectedPlanData.isTrial ? 0 : Number(
  ...
  const handleConfirm = async () => {
    ...
    setLoading(true);
    try {
      const userId = useDoctorRegistrationStore.getState().userId;

      if (!userId) {
        throw new Error("User ID not found. Please restart registration.");
      }

      // Activate Doctor Profile
      const res = await activateDoctor(userId);

      if (res && (res.success || res.message?.toLowerCase().includes('activated'))) {
        useToastStore.getState().addToast({
          title: "Success",
      ...
      useToastStore.getState().addToast({
        title: "Error",
        message: error.message || "Failed to process payment/activation.",
        type: "error"
      });
    } finally {
      setLoading(false);
                  ...
                  <div className="bg-card rounded-xl border border-border p-6">
                    ...
                    <div className="space-y-3">
                      ...
                      <button
                        ...
                        className="w-full mt-6 bg-blue-primary250 hover:bg-blue-primary300 text-white py-3 rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                      >
