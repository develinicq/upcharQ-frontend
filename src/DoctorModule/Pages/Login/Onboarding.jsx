...
const Onboarding = ({ onContinue }) => {
  ...
  // Mock data for dev if no code provided (remove in prod if needed, or keep for easy dev)
  useEffect(() => {
    // If no code, use mock data to show the UI
    if (!searchParams.get('code') && !details) {
      setDetails({
        user: {
          firstName: 'Milind',
          lastName: 'Chauhan',
  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch((err) => {
        ...
        const msg = err?.response?.data?.message || (status ? `Request failed with status ${status}` : 'Failed to fetch invitation details.');
        setError(msg);
        setLoading(false);
      });
  }, [token, searchParams]);
    ...
    <div className="min-h-screen flex w-full bg-white font-sans overflow-hidden">
      ...
      <div className="hidden lg:flex w-1/2 relative flex-col items-center justify-center overflow-hidden">
        ...
        <div className="absolute top-8 left-0 w-full text-center z-20">
          <h1 className="text-3xl text-white font-light tracking-wider">Upchar-Q</h1>
        </div>
        ...
        <div className="relative z-10 flex flex-col items-center text-center p-8 mt-10">
          ...
          <div className="flex flex-col gap-2 text-white">
            <h2 className="text-3xl font-semibold tracking-wide">Welcome To Upchar-Q</h2>
        ...
        <div className="flex-1 flex flex-col justify-center p-8 max-w-[420px] mx-auto w-full items-center">
          <div className="mb-6 text-center">
            <h2 className="text-xl font-bold text-gray-700 mb-1">
              Welcome, {user?.firstName ? `Dr. ${user.firstName} ${user.lastName || ''}` : 'Dr. Milind Chauhan'}
            </h2>
