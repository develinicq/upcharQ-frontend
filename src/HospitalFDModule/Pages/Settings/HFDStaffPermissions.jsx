...
const StaffUI = () => {
  ...
  const { user, clinicId: authClinicId } = useHospitalFrontDeskAuthStore();
  const { clinic } = useClinicStore();

  const resolveClinicId = () => authClinicId || user?.clinicId || user?.clinic?.id || clinic?.id || clinic?.clinicId;
  ...
  useEffect(() => { loadData(); }, [user, clinic, authClinicId]);
...
export default function HFDStaffPermissions() {
