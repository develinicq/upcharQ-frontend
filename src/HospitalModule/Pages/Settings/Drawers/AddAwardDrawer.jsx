...
import { addHospitalAwardForAdmin, updateHospitalAwardForAdmin } from '../../../../services/hospitalService';
import useToastStore from '../../../../store/useToastStore';
import UniversalLoader from '../../../../components/UniversalLoader';
import useHospitalAuthStore from '../../../../store/useHospitalAuthStore';

export default function AddAwardDrawer({ open, onClose, onSuccess, mode = "add", initial = {} }) {
    ...
    useEffect(() => {
        if (!open) return;
        setTitle(initial?.awardName || "");
        setIssuer(initial?.issuerName || "");
        setAssoc(initial?.associatedWith || "");
        setDate(initial?.issueDate ? String(initial.issueDate).split("T")[0] : "");
        setUrl(initial?.awardUrl || "");
    ...
    const hasChanges = useMemo(() => {
        ...
        const initialTitle = initial?.awardName || "";
        const initialIssuer = initial?.issuerName || "";
        const initialAssoc = initial?.associatedWith || "";
        const initialDate = initial?.issueDate ? String(initial.issueDate).split("T")[0] : "";
        const initialUrl = initial?.awardUrl || "";
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (mode === "edit" && initial?.id) {
                ...
                res = await updateHospitalAwardForAdmin(hospitalId, initial.id, partialPayload);
            } else {
                const payload = {
                ...
                res = await addHospitalAwardForAdmin(hospitalId, payload);
            }
