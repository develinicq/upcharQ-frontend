...
import { addHospitalAccreditationForAdmin, updateHospitalAccreditationForAdmin } from "@/services/hospitalService";
import useToastStore from "@/store/useToastStore";
import UniversalLoader from "@/components/UniversalLoader";
import useHospitalAuthStore from "@/store/useHospitalAuthStore";

export default function AccreditationDrawer({ open, onClose, onSuccess, mode = "add", initial = {} }) {
    ...
    useEffect(() => {
        ...
        setIssueDate(initial?.issueDate ? String(initial.issueDate).split("T")[0] : "");
        setExpiryDate(initial?.expiryDate ? String(initial.expiryDate).split("T")[0] : "");
    ...
    const hasChanges = React.useMemo(() => {
        ...
        const initialIssueDate = initial?.issueDate ? String(initial.issueDate).split("T")[0] : "";
        const initialExpiryDate = initial?.expiryDate ? String(initial.expiryDate).split("T")[0] : "";
    ...
    const save = async () => {
        ...
        try {
            ...
            if (mode === "edit" && initial?.id) {
                ...
                res = await updateHospitalAccreditationForAdmin(hospitalId, initial.id, partialPayload);
            } else {
                const payload = {
                ...
                res = await addHospitalAccreditationForAdmin(hospitalId, payload);
            }
                    ...
                    <div className="relative flex-1">
                        ...
                        {showIssueCalendar && (
                            <div className="shadcn-calendar-dropdown absolute right-1 top-full z-[10000] mt-1 bg-white border border-gray-200 rounded-xl shadow-2xl p-2">
                                <ShadcnCalendar mode="single" selected={issueDate ? new Date(issueDate) : undefined} onSelect={(d) => { if (d) { const yyyy = d.getFullYear(); const mm = String(d.getMonth() + 1).padStart(2, "0"); const dd = String(d.getDate()).padStart(2, "0"); setIssueDate(`${yyyy}-${mm}-${dd}`); } setShowIssueCalendar(false); }} captionLayout="dropdown" fromYear={1900} toYear={new Date().getFullYear()} className="rounded-lg p-1" />
                            </div>
                        )}
                    </div>
                    <div className="relative flex-1">
                        ...
                        {showExpiryCalendar && (
                            <div className="shadcn-calendar-dropdown absolute z-[10000] mt-1 bg-white border border-gray-200 rounded-xl shadow-2xl p-2 right-0">
                                <ShadcnCalendar mode="single" selected={expiryDate ? new Date(expiryDate) : undefined} onSelect={(d) => { if (d) { const yyyy = d.getFullYear(); const mm = String(d.getMonth() + 1).padStart(2, "0"); const dd = String(d.getDate()).padStart(2, "0"); setExpiryDate(`${yyyy}-${mm}-${dd}`); } setShowExpiryCalendar(false); }} captionLayout="dropdown" fromYear={1900} toYear={new Date().getFullYear() + 10} className="rounded-lg p-1" />
                            </div>
                        )}
                    </div>
                </div>
