...
import { getPatientDetailsForHospital } from "../../../services/patientService";
...
export default function PatientDetailsDrawer({
    ...
    useEffect(() => {
        const idToFetch = patient?.id || patient?.patientId || patient?.userId;
        if (isOpen && idToFetch && hospitalId) {
            setLoadingDetails(true);
            getPatientDetailsForHospital(hospitalId, idToFetch)
                .then(res => {
    ...
    const primaryLang = contactInfo.primaryLanguage || "-";
    const secondaryLangs = contactInfo.secondaryLanguages || "-";
    ...
    const activeProblems = details?.activeProblems || [];
    const recentAppointments = details?.recentAppointments || [];
    const lastRecordedVitals = details?.lastRecordedVitalsAndBiometrics;
                                    ...
                                    <div className="flex flex-col gap-4 pt-3 px-3 pb-3">
                                        ...
                                        <div className="flex flex-col gap-1">
                                            ...
                                            <div className="border-t border-secondary-grey100/50 pt-2 flex flex-col gap-2">
                                                ...
                                                <div className="flex items-center gap-2 text-sm">
                                                    ...
                                                    <span className="text-secondary-grey300">{[primaryLang, secondaryLangs].filter(Boolean).join('/') || "-"}</span>
                                                </div>
                                            </div>
                                        </div>
                                                ...
                                                {recentAppointments.length > 0 ? (
                                                    <>
                                                        ...
                                                        <div className="flex items-center justify-between">
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                <AvatarCircle size='xs' color='orange' name={recentAppointments[0].doctor?.charAt(0) || 'D'} />
                                                                <div className="col-span-2 font-medium">
                                                                    {recentAppointments[0].doctor || "-"}
                                                                </div>
                                                            </div>
                                            ...
                                            {lastRecordedVitals ? (
                                                <div>
                                                    <div className="text-xs text-gray-500 mb-1">
                                                        {(lastRecordedVitals.recordedOn &&
                                                            new Date(
                                                                lastRecordedVitals.recordedOn
                                                            ).toLocaleDateString("en-GB")) ||
                                                            "Recorded on —"}
                                                        {lastRecordedVitals.recordedBy
                                                            ? ` by ${lastRecordedVitals.recordedBy.name ||
                                                            lastRecordedVitals.recordedBy
                                                            }`
                                                            : ""}
                                                    </div>
                                                    <div className="text-sm text-gray-700 space-y-1">
                                                        {Array.isArray(lastRecordedVitals.entries) &&
                                                            lastRecordedVitals.entries.length > 0
                                                            ? lastRecordedVitals.entries.map((e, i) => (
                                                            ...
                                                            : Object.keys(lastRecordedVitals)
                                                                .filter(
                                                                    (k) =>
                                                                        ![
                                                                            "recordedOn",
                                                ...
                                                <div className="text-xs text-gray-400 p-2">No recorded vitals</div>
                                            )}
                                        </div>
                                        ...
                                        <div className="flex flex-col gap-1 pt-1">
                                            ...
                                            <div className="border-t border-secondary-grey100/50 pt-2 flex flex-col gap-[10px] text-sm text-secondary-grey300">
                                                {activeProblems.length > 0 ? activeProblems.map((prob, idx) => (
                                                    <div key={idx} className="flex items-center justify-between">
                                                        <div className="text-gray-600">{prob.name || prob.problem || "Problem"}</div>
                                                        <div>
                                                            <Badge type="ghost" color="gray" showText="Height">
                                                                {prob.status || "Active"}
                                                            </Badge>
                                                        </div>
                                                    </div>
                                                )) : (
                                                    <div className="text-xs text-gray-400">No active problems</div>
                                                )}
                                            </div>
                                        </div>
                            ...
                            {activeTab === "Demographics" && (
                                <div className="flex flex-col gap-4 pt-3  px-3 pb-3">
                                    ...
                                    <SectionCard title="Address Details">
                                        <div className=" text-blue-primary300  text-sm leading-[22px] tracking-[0px]">
                                            Permanent Address
                                        </div>
                                        <Row label="Address:" value={addressDetails.address || "-"} />
                                        <Row label="City:" value={addressDetails.city || "-"} />
                                        <Row label="State:" value={addressDetails.state || "-"} />
                                        <Row label="Zip Code:" value={addressDetails.zipCode || "-"} />
                                    </SectionCard>
                                    ...
                                    >
                                        {dependents.length > 0 ? dependents.map((dep, idx) => (
                                            <div key={idx} className="flex items-center gap-3 py-1 text-sm text-secondary-grey400">
                                                <AvatarCircle size='s' name={dep.name?.charAt(0) || 'D'} />
                                                <div className="flex flex-col ">
                                                ...
                                                <div className="ml-auto">
                                                    <button className="p-1.5 rounded hover:bg-gray-100">
                                                        <img src="/icons/Menu Dots.svg" alt="options" width={16} height={16} />
                                                    </button>
                                                </div>
                                            </div>
                                        )) : (
