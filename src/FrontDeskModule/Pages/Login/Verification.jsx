...
export default function FDVerification({ onVerified, invitationId, mobileChallengeId, emailChallengeId, password, confirmPassword, userId, hospitalId, userDetails }) {
          ...
          <div className="p-3 sm:p-4">
            <div className="text-center mb-2">
              ...
              <p className="text-gray-500 text-[10.5px]">
                Enter 6-digit OTP sent on your email ({user.emailId ? user.emailId.replace(/^(.{3})(.*)(@.*)$/, "$1***$3") : '...'}) and mobile number ({user.phone ? user.phone.replace(/\D/g, '').slice(-10).replace(/(\d{2})(\d+)(\d{2})$/, "$1******$3") : '...'})
              </p>
            </div>
