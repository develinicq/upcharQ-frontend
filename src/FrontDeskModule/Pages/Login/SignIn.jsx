...
export default function FDSignIn() {
  ...
  // Use Front Desk store as primary for this page
  const { login, verifyOtp } = useFrontDeskAuthStore();
  ...
  const handleAuthSuccess = (res) => {
    ...
    const excludedRoles = ['CLINIC_STAFF', 'HOSPITAL_STAFF']; // Roles that are natively handled here

    // Handle Doctor Role
    if (roles.includes("DOCTOR")) {
      useFrontDeskAuthStore.getState().clearAuth();
      const { setToken, setRoleNames, setUser } = useDoctorAuthStore.getState();
      ...
      addToast({
        ...
        message: "Redirecting to doctor dashboard...",
        type: "success",
        duration: 2000
      });
    ...
    // Handle Hospital Admin Role
    if (roles.includes("HOSPITAL_ADMIN")) {
      useFrontDeskAuthStore.getState().clearAuth();
      // Assume redirection logic for hospital admin
      addToast({
        title: "Access Denied",
    ...
    // Handle Front Desk Roles
    if (roles.includes("CLINIC_STAFF")) {
      // Already set in store by login/verifyOtp if logic allows, or we set it manually
      // But login/verifyOtp in useFrontDeskAuthStore handles CLINIC_STAFF and HOSPITAL_STAFF

      // Ensure other auths are cleared
    ...
    if (roles.includes("HOSPITAL_STAFF")) {
      ...
      // Although the store handles it, we might want to redirect explicitly
      // If useFrontDeskAuthStore handles HOSPITAL_STAFF too, then we are good.
      // But usually Hospitality Front Desk has its own store? 
      // Yes: useHospitalFrontDeskAuthStore.

      // If useFrontDeskAuthStore handles generic staff, good.
      // But let's check if we need to switch store.
      const { setToken, setRoleNames, setUser } = useHospitalFrontDeskAuthStore.getState();
      setToken(res.data.token);
      setRoleNames(roles);
      setUser(res.data.user);

      // Clear generic FD store if it's strictly for Clinic
      // But if useFrontDeskAuthStore is shared, it's fine.
      // Let's assume split:
      useFrontDeskAuthStore.getState().clearAuth();
    ...
    try {
      ...
      if (res.success) {
        if (loginMode === 'password') {
          ...
          setMaskedPhone(res.data.data.userName);
          addToast({
            title: "OTP Sent",
    ...
    <div className="min-h-screen flex flex-1 bg-secondary-grey50 p-3 w-full">
      ...
      <div className=" flex flex-col gap-16  bg-white px-3 p-3 w-1/2">
        <div className=" flex flex-col items-start gap-1 px-3">
          ...
          <div className="bg-warning-50 px-1 w-fit py-[2px] min-w-[18px] text-warning-400 text-[12px]">For Front Desk</div>
        </div>
        ...
        <div className="flex items-center justify-center">
          <div className="w-full max-w-[500px] flex flex-col gap-6">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-[14px] text-secondary-grey300 leading-tight">Login to access front desk module.</p>
            </div>
