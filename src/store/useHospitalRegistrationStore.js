...
const useHospitalRegistrationStore = create(
  persist(
    (set, get) => ({
      ...initialState,
        ...
        try {
          ...
          const sanitizeDocuments = (docs) => {
            ...
            for (const d of docs) {
              ...
              // Fix legacy type mismatch if persisted
              if (type === 'NABH' || type === 'NABH_ACCREDITATION') type = 'NABH_ACCREDITATION_PROOF';
    ...
    {
      name: 'hospital-registration-storage',
      partialize: (state) => ({ ...state, loading: false, error: null, success: false }),
    }
  )
);
